...
\begin{frame}{Mutual Fund Marketing in Practice}

\begin{columns}[T]

\begin{column}{0.31\textwidth}
  \centering
  \textbf{\small (1) Family Brand --- TV}\\[4pt]
  \includegraphics[width=\textwidth, height=3.8cm, keepaspectratio]{pictures/Vangard.png}\\[5pt]
  {\footnotesize \textbf{Vanguard} national TV campaign (2022): ``I Value Feeling Valued.''
  Pure brand advertising --- no specific fund featured.
  Builds institutional identity across 30M+ investors.}
\end{column}

\begin{column}{0.31\textwidth}
  \centering
  \textbf{\small (2) Star Manager --- Print}\\[4pt]
  \includegraphics[width=\textwidth, height=3.8cm, keepaspectratio]{pictures/PeterLynch.png}\\[5pt]
  {\footnotesize \textbf{Fidelity} print ad (1998): Peter Lynch endorses Fidelity
  Roth IRA conversion. Family credibility via star manager --- promotes a
  specific service, not a fund.}
\end{column}

\begin{column}{0.31\textwidth}
  \centering
  \textbf{\small (3) Star Manager --- Digital}\\[4pt]
  \includegraphics[width=\textwidth, height=3.8cm, keepaspectratio]{pictures/Jingshun.png}\\[5pt]
  {\footnotesize \textbf{景顺长城} (Invesco China) digital ad: star manager
  刘彦春 (Yanchun Liu) promotes a specific fund with 3-year return data.
  \hl{Product-level}: direct conversion focus.}
\end{column}

\end{columns}

\vspace{0.15cm}
\muted{\small Marketing ranges from family-level brand-building \emph{(1, 2)} to product-level performance showcasing \emph{(3)}.
\textit{Our question:} Does any of this translate to \emph{net new} assets at the family level?}
...
\begin{wideitemize}
  ...
  \textbf{family level}, using \hl{25,685} livestreaming events across
  \hl{180} families and \hl{5,809} funds

  \item We analyze \hl{795 transcripts} with a large language model to
  understand what communication strategies attract outside capital

  \item We examine \textbf{competitive dynamics}: do families livestream to
  grow, or to defend existing market share?
\end{wideitemize}

\vspace{0.25cm}
\begin{block}{Setting}
  China's Tiantian Fund platform provides a uniform observation window
  across all major families, with verified treatment status (``featured'' funds
  in the shopping cart) and precise broadcast timestamps.
\end{block}
...
\begin{enumerate}\addtolength{\itemsep}{0.18cm}

  \item[\hlg{(1)}] \textbf{Zero at the family level.}
  Livestreaming generates large fund-level inflows (\hl{$+$14 pp}) but
  \hl{zero} family-level flow or revenue change.
  ...
  rational learning. Livestreaming acts as an \hl{internal open market operation}.

  \item[\hlg{(3)}] \textbf{Defensive arms race.}
  When rivals increase livestreaming by 1 SD, non-livestreamers
  lose $\approx$27 pp in flows. Families livestream to
  \hl{defend market share}, not to expand it --- a prisoner's dilemma.

  \item[\hlg{(4)}] \textbf{Hard information wins.}
  Families that do attract outside capital emphasize
  \hl{past performance} and \hl{social proof} ---
  not emotional appeals or engagement style.
...
\begin{itemize}
  ...
  \item \hl{Compliance approval obtained days in advance}:
...
\begin{frame}{Livestream Marketing: A New Ecosystem}

\textbf{A new marketing ecosystem:}
\begin{itemize}
  \item Third-party platforms (Tiantian, Alipay, JD Finance)
  host dedicated livestream portals for all families
  \item Fund families maintain official channels with
  \hl{verified broadcaster} status
  \item Broadcasts are scheduled, indexed, and archived ---
  providing a complete observational record
\end{itemize}

\vspace{0.4cm}
\begin{alertblock}{Key shift}
  From \textit{intermediary-driven} distribution (brokers, bank advisors)
  to \textit{direct-to-investor} digital marketing --- families now
  communicate directly with retail investors in real time.
\end{alertblock}
...
\begin{frame}{Scale of Livestream Marketing}

\textbf{Scale of the phenomenon:}
\begin{itemize}
  \item \hl{25,685} livestreaming events in our sample
  \item July 2020 -- June 2025 (20 quarters)
  \item Families broadcast \hl{multiple times per month}
  \item Typical broadcast: 1--2 hours; live audience ranges
  from hundreds to tens of thousands of viewers
\end{itemize}

\vspace{0.35cm}
\textbf{Our platform (Tiantian Fund):}
\begin{itemize}
  ...
  \item Hosts livestream channels for all major families,
  giving us a \hl{uniform cross-family} observation window
\end{itemize}
...
\begin{frame}{Adoption and Sample}

\textbf{Adoption over time:}
\begin{itemize}
  \item Near-zero livestream activity before July 2020
  \item Rapid diffusion: most families adopted within
  2--3 quarters of the first movers
  \item By 2021--2022, livestreaming was universal among
  active equity fund families
  \item Adoption is \hl{staggered}: families enter at
  different quarters --- enables DiD identification
\end{itemize}

\vspace{0.4cm}
\begin{block}{Our sample}
  \hl{90,850} fund-quarter observations; unbalanced panel
  2020~Q3--2025~Q2. SEs clustered at the family level.
\end{block}
...
\begin{frame}{Structure of a Livestream Event}
...
\begin{itemize}
  ...
  \item The selected funds appear in the \hl{shopping cart}
  (``Product Basket'') embedded in the livestream interface
  \item This creates sharp \hl{within-family variation} in
  promotional intensity --- ideal for identification
\end{itemize}

\vspace{0.35cm}
\textbf{Pre-planned, not spontaneous:}
\begin{itemize}
  \item Compliance approval is obtained \hl{days in advance}
  \item Topic, featured funds, and risk disclosures are
  all fixed before the broadcast
  \item Eliminates concern that fund selection reflects
  real-time information shocks
\end{itemize}
...
\begin{frame}{Treatment Definition and Identification}

\begin{block}{Treatment definition}
  A fund is \hl{``featured''} in quarter $q$ if any share class
  appears in the shopping cart during any broadcast that quarter.

  \medskip
  A fund is \hl{``non-featured''} if its family broadcasts
  but the fund is not selected.

  \medskip
  Within-family, within-quarter comparison.
\end{block}

\vspace{0.4cm}
\begin{alertblock}{Identification}
  Featured and non-featured funds belong to the \emph{same}
  family in the \emph{same} quarter --- family $\times$ quarter
  FE absorbs all common shocks. The only difference is
  \hl{which fund the family chose to promote}.
\end{alertblock}
...
\begin{itemize}
  ...
  \item Viewers can ask questions in real-time via the comment interface
\end{itemize}
...
\begin{itemize}
  ...
  \item Viewers tap \hl{``Subscribe''} to purchase directly,
  without leaving the livestream
\end{itemize}
\end{column}
...
\medskip
\begin{itemize}
  \item \textbf{Fund managers:} provide technical commentary
  and portfolio outlook
  \item \textbf{Professional hosts:} deliver polished presentations;
  common at large families
  \item \textbf{Guest speakers:} economists, market analysts,
...
\begin{column}{0.31\textwidth}
  ...
  {\footnotesize\muted{Host video, risk disclosure banners, product basket icon, engagement tools}}
\end{column}
\begin{column}{0.31\textwidth}
  \centering
...
\begin{itemize}
  ...
  \item Records: broadcast date, duration, host identity,
  featured funds (shopping cart contents)
\end{itemize}

\vspace{0.25cm}
\textbf{Fund universe:}
\begin{itemize}
  \item \hl{5,809} actively managed equity and hybrid funds
  \item \hl{180} fund families
  \item \hl{90,850} fund-quarter observations
  \item Excluded: index, closed-end, QDII, and umbrella funds
\end{itemize}
\end{column}
\begin{column}{0.50\textwidth}
\textbf{Descriptive highlights:}
\begin{itemize}
  \item Mean quarterly fund flow: 0.38\%, SD: 42\%
  \item \hl{14\%} of fund-quarters are ``featured''
  \item Featured funds outperform non-featured siblings
  in the months before the event
\end{itemize}

\vspace{0.25cm}
\textbf{Fund flow data:}
\begin{itemize}
  \item Total Net Assets (TNA) from Wind Financial
  \item Quarterly returns from fund prospectuses
  \item Fund characteristics: inception date, manager,
  benchmark, share class structure
\end{itemize}

\vspace{0.2cm}
\begin{block}{Panel structure}
  Unbalanced fund-quarter panel, 2020 Q3 -- 2025 Q2.
  SEs clustered at the family level throughout.
\end{block}
\end{column}
\end{columns}
...
\begin{frame}{Key Variables: Fund Flow}
...
\textbf{Why return-adjust?}

\medskip
Raw TNA changes mix investor flows with capital gains.
Stripping out returns isolates \hl{investor decisions}
--- new subscriptions minus redemptions.
\end{column}
\end{columns}
...
\]
\vspace{0.1cm}
\muted{\small where $\Delta\text{TNA}_{i,q}^{\text{adj}} = \text{TNA}_{i,q} - \text{TNA}_{i,q-1}(1+r_{i,q})$}
\end{column}
\begin{column}{0.44\textwidth}
\textbf{Why aggregate to the family?}

\medskip
If featured funds gain and non-featured lose by the
\emph{same} amount, \hl{FamilyFlow = 0} --- even though
every fund-level regression shows a positive coefficient.
\end{column}
\end{columns}

\vspace{0.4cm}
\begin{block}{Our main test}
  Is $\widehat{\beta}$ in the family-level regression
  statistically and economically different from zero?
\end{block}
...
\begin{frame}{Key Variables: Treatment Indicators}

\begin{columns}[T]
\begin{column}{0.52\textwidth}
\textbf{Three treatment indicators:}

\medskip
\begin{itemize}\addtolength{\itemsep}{8pt}
  \item \textbf{Fund Livestream}$_{i,q}$: fund $i$ appears in
  the shopping cart during any broadcast in quarter $q$
  (\hl{``featured''})

  \item \textbf{Family Livestream (Oth)}$_{i,q}$: family $f$
  broadcasts in quarter $q$ but fund $i$ is \emph{not}
  featured --- the \hl{sibling treatment}; captures
  cannibalization of non-featured funds

  \item \textbf{Family Livestream}$_{f,q}$: family $f$ holds
  \emph{any} broadcast in quarter $q$ --- the
  \hl{family-level indicator} used in the aggregate test
\end{itemize}
\end{column}
...
\begin{tabular}{lc}
  ...
  Statistic & Value \\
  \midrule
  Fund-quarters featured & 14\% \\
  Fund-quarters with sibling featured & 38\% \\
  Family-quarters with any broadcast & 52\% \\
  \midrule
  Avg.\ broadcasts per family per quarter & 4.3 \\
  Avg.\ funds per broadcast & 2.1 \\
  \bottomrule
\end{tabular}}

\vspace{0.3cm}
\begin{alertblock}{Within-family variation}
  Featured and non-featured funds are in the \emph{same}
  family, \emph{same} quarter --- family FE absorbs
  all common shocks.
\end{alertblock}
\end{column}
\end{columns}
...
\begin{frame}{Communication Strategies: Methodology}
...
\begin{frame}{Communication Strategies: Results}

\begin{columns}[T]
\begin{column}{0.50\textwidth}
\textbf{Only 2 of 16 dimensions differ:}
...
\begin{column}{0.48\textwidth}
\begin{block}{Interpretation}
  Investors respond to \textbf{verifiable ``hard'' information}
  --- past returns and third-party validation ---
  not emotional appeals or engagement style.
\end{block}

\vspace{0.3cm}
The 14 insignificant dimensions include sentiment,
emotional intensity, sales pressure, and educational
content --- suggesting that \hl{style} does not
distinguish high- from low-flow families.

Only \hl{substance} (track record and social proof)
attracts outside capital.
\end{column}
\end{columns}
